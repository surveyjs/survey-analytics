...
test("useNamesAsTitles option", () => {
  ...
  expect(JSON.parse(JSON.stringify(columns))).toEqual(
    [{ "headerSort": false, "download": false, "resizable": false, "minWidth": 60, "width": 60 }, { "field": "str", "title": "String", "widthShrink": 1, "visible": true, "headerSort": false, "formatter": "plaintext", minWidth: 248, headerTooltip: true }]
  );

  tabulator = new Tabulator(survey, [], <any>{ useNamesAsTitles: true });
  columns = <any>tabulator.getColumns();
  expect(JSON.parse(JSON.stringify(columns))).toEqual(
    [{ "headerSort": false, "download": false, "resizable": false, "minWidth": 60, "width": 60 }, { "field": "str", "title": "str", "widthShrink": 1, "visible": true, "headerSort": false, "formatter": "plaintext", minWidth: 248, headerTooltip: true }]
  );
});
